    ...
    execute: async (message, args, client) => {
        ...
        setTimeout(() => {
            ...
            if (!args.length) {
                const cmdHelpEmbed = new Discord.MessageEmbed()
                    .setAuthor('AI-Chan (' + VERSION + ') - Information', IMAGE_INFOEMBED)
                    .addField('Error', 'Use `' + PREFIX + 'quitmessage' + ' [quitmessage]` to set the Quit Message', false)
                    .addField('Optional', 'Use `' + PREFIX + 'quitmessage' + ' status` to view the current Quit Message', false)
                    .setColor(embedError);
                client.channels.cache.get(channelid).send(cmdHelpEmbed);
                return
            }
            ...
            mysqlhandler.con.query(`SELECT * FROM botsettings WHERE id = '${message.guild.id}'`, (err, rows) => {
                ...
                if (rows.length < 1) {
                    ...
                    if (value === null) {
                        ...
                        sql = `UPDATE botsettings SET quitMessage = '` + messagevalue + `' WHERE id='${message.guild.id}'`;
